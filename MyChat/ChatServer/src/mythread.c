...
*/
int myread(struct msg *msg,int lisfd)
{
	if(NULL == msg)
	{
		printf("msg NULL!\n");
	...
	message::UserLoginRequest reqest;
	reqest.ParseFromArray(data,msghead.size - sizeof(MY_MSG_HEAD));

	printf("msghead id:%d,size:%d\n",msghead.msgid,msghead.size );
	printf("reqest name:%s,pwd:%s\n",reqest.login_name().c_str(),reqest.login_password().c_str());
